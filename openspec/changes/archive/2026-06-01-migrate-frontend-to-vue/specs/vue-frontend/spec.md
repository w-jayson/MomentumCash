...
O estado das transações e categorias SHALL ser persistido automaticamente em localStorage sempre que houver mudanças.

#### Scenario: Data survives page reload
- **WHEN** o usuário cria transações e recarrega a página
- **THEN** as transações são carregadas do localStorage e exibidas corretamente
